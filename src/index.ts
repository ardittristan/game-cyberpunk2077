...
function findGame() {
  try {
    const instPath = winapi.RegGetValue("HKEY_LOCAL_MACHINE", "SOFTWARE\\WOW6432Node\\GOG.com\\Games\\" + GOGAPP_ID, "PATH");
    if (!instPath) {
    ...
    return util.GameStoreHelper.findByAppId([STEAMAPP_ID, GOGAPP_ID, EPICAPP_ID]).then((game: IGameStoreEntry) => game.gamePath);
  }
}

function requiresGoGLauncher() {
  return util.GameStoreHelper.isGameInstalled(GOGAPP_ID, "gog").then((gog: IGameStoreEntry) => (gog ? { launcher: "gog", addInfo: GOGAPP_ID } : undefined));
}
